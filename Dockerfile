...
RUN apk update \
    && apk add --no-cache --purge git curl upx gcompat
...
RUN GONOSUMCHECK='*' go install github.com/infobloxopen/protoc-gen-gorm@v0.20.4
...
# Build protoc-gen-swagger from atlas-patch fork, pinned to the commit used in v21.8
RUN git clone --single-branch -b atlas-patch \
      https://github.com/infobloxopen/grpc-gateway.git /tmp/grpc-gateway && \
    cd /tmp/grpc-gateway && git checkout 941a1b19 && \
...
RUN GONOSUMCHECK='*' go mod download \
      ...
      github.com/infobloxopen/protoc-gen-gorm@v0.20.4 \
...
RUN mkdir -p /out/go/src && MOD=${GOPATH}/pkg/mod && \
    ...
    copy_mod_protos "$MOD/github.com/infobloxopen/protoc-gen-gorm@v0.20.4" "github.com/infobloxopen/protoc-gen-gorm" && \
